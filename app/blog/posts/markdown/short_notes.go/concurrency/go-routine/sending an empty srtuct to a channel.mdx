...
```
First we create a semaphore, a [[markdown//concurrency/go-routine/buffered-chanel]], and everty time the go function executes send that empty struct, when reach the limit of the buffer, it finnaly awaits blocking the channel `<-semaphore`